...
class SAC():
    ...
    def train(self):
        ...
        try:
            checkpoint = torch.load(self.restore_path + '/state.pt')
            # print(checkpoint)
            # Episode and frames
            ...
            self.soft_q_net.load_state_dict(checkpoint['soft_q_net'])
            self.policy_net.load_state_dict(checkpoint['policy_net'])

            # Optimizers
            self.value_optimizer.load_state_dict(checkpoint['value_optimizer'])
            self.soft_q_optimizer.load_state_dict(checkpoint['soft_q_optimizer'])
            self.policy_optimizer.load_state_dict(checkpoint['policy_optimizer'])

            self.replay_buffer = checkpoint['replay_buffer']
        except BaseException:
            print('Não foi possível carregar um modelo SAC pré-existente')
            None
...
def main(args):
    ...
    env = DroneEnv(random=args.env_reset_mode, device=device, restore_path=restore_path, headless=True, seed=args.seed,
